...
class KAN(nn.Module):
    ...
    def update_grid_from_samples(self, x):
        ...
        for l in range(self.depth):
            self.forward(x)
            self.act_fun[l].update_grid_from_samples(self.acts[l])  # ????更新grid???
        ...
        for l in range(depth):
            ...
            for i in range(self.width[l]):
                for j in range(self.width[l + 1]):
                    ...
                    # plt.gca().spines.values[:].set_color(color)
                    # plt.gca().spines[:].set_color(color)  # 默认代码--liubing-20242515
    ...
    def train(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1., stop_grid_update_step=50, batch=-1,
              small_mag_threshold=1e-16, small_reg_factor=1., metrics=None, sglr_avoid=False, save_fig=False, in_vars=None, out_vars=None, beta=3, save_fig_freq=1, img_folder='./video', device='cuda'):
        '''
        training
