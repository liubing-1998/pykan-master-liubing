...
class KANLayer(nn.Module):
    ...
    def initialize_grid_from_parent(self, parent, x):
        ...
        x_pos = parent.grid
        sp2 = KANLayer(in_dim=1, out_dim=self.size, k=1, num=x_pos.shape[1] - 1, scale_base=0.).to(self.device)  #换成MLP
        sp2.coef.data = curve2coef(sp2.grid, x_pos, sp2.grid, k=1)  # -
        y_eval = coef2curve(x_eval, parent.grid, parent.coef, parent.k, device=self.device)  # ??? 用MLP算
        percentile = torch.linspace(-1, 1, self.num + 1).to(self.device)
        self.grid.data = sp2(percentile.unsqueeze(dim=1))[0].permute(1, 0)
        self.coef.data = curve2coef(x_eval, y_eval, self.grid, self.k, self.device)  # fit new MLP
        # KANLayer 中的B-spline 换成MLP，
